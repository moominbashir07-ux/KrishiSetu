...
test('KrishiSetu Master Control Center V5 Forensic Audit & Production Suite', async (t) => {
  ...
  const adminToken = generateToken({ id: 'ADM_V5', role: 'admin', contact: 'admin' });
  ...
  await t.test('2. Cash on Delivery (COD) Checkout creates order with payment_status=cod and reduces stock', async () => {
    await db.query("INSERT INTO products (id, seller_id, name, category, price, quantity, status) VALUES ('P105', 'S101', 'Fresh Wheat', 'Grains', 30.00, 200.00, 'active')").catch(() => {});
    ...
    await db.query("INSERT INTO cart_items (id, cart_id, product_id, quantity) VALUES ('CI_V5', 'CART_C101', 'P105', 10.00)").catch(() => {});
  ...
  await t.test('3. Online Payment requires mandatory Transaction ID (min 5 chars)', async () => {
    const createRes = await makeRequest('/api/orders', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { productId: 'P105', quantity: 2, payment_method: 'upi_qr' }
    });
    ...
    const passRes = await makeRequest(`/api/orders/${orderId}/verify-payment`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { transactionId: 'UTR9988776655' }
    });

    assert.equal(passRes.status, 200);
    assert.equal(passRes.body.paymentStatus, 'submitted');
    assert.equal(passRes.body.transactionId, 'UTR9988776655');
  });

  await t.test('4. Seller Payment Verification: Seller approval confirms order & sets payment_status=verified', async () => {
    const createRes = await makeRequest('/api/orders', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { productId: 'P105', quantity: 3, payment_method: 'upi_qr' }
    });
    const orderId = createRes.body.orders[0].id;
    ...
    await makeRequest(`/api/orders/${orderId}/verify-payment`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { transactionId: 'UTR5544332211' }
    });
  ...
  await t.test('5. Seller Payment Verification: Seller rejection sets payment_status=rejected and records reason', async () => {
    const createRes = await makeRequest('/api/orders', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { productId: 'P105', quantity: 1, payment_method: 'upi_qr' }
    });
    const orderId = createRes.body.orders[0].id;
    ...
    await makeRequest(`/api/orders/${orderId}/verify-payment`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { transactionId: 'FAKE_UTR_000' }
    });

    const rejectRes = await makeRequest(`/api/orders/${orderId}/seller-verify-payment`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${seller1Token}`, 'Content-Type': 'application/json' },
      body: { action: 'reject', reason: 'Bank credit missing.' }
    });

    assert.equal(rejectRes.status, 200);
    assert.equal(rejectRes.body.paymentStatus, 'rejected');
    assert.equal(rejectRes.body.reason, 'Bank credit missing.');
  });

  await t.test('6. IDOR Security: Seller B cannot verify Seller A payment', async () => {
    const createRes = await makeRequest('/api/orders', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { productId: 'P105', quantity: 1, payment_method: 'upi_qr' }
  ...
  await t.test('7. GET /api/admin/users/:id/profile returns full user inspector details', async () => {
  ...
  await t.test('8. GET /api/admin/mandi-health returns API health status', async () => {
