...
test('KrishiSetu Master Control Center V13 Final Production Hardening Suite', async (t) => {
  ...
  await t.test('2. GET /api/admin/build-info returns build commit 5e791c3 metadata', async () => {
  ...
  await t.test('3. GET /js/api.js returns JavaScript file containing AuthService.getToken', async () => {
    const res = await makeRequest('/js/api.js?v=5e791c3');
    assert.equal(res.status, 200);
    assert.equal(typeof res.raw, 'string');
  ...
  await t.test('4. Cash on Delivery (COD) Checkout creates order with payment_status=cod and reduces stock', async () => {
    await db.query("INSERT INTO products (id, seller_id, name, category, price, quantity, status) VALUES ('P112', 'S101', 'Fresh Mangoes', 'Fruits', 150.00, 200.00, 'active')").catch(() => {});
    ...
    await db.query("INSERT INTO cart_items (id, cart_id, product_id, quantity) VALUES ('CI_V13', 'CART_C101', 'P112', 10.00)").catch(() => {});
  ...
  await t.test('5. Online Payment requires mandatory Transaction ID (min 5 chars)', async () => {
    const createRes = await makeRequest('/api/orders', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { productId: 'P112', quantity: 2, payment_method: 'upi_qr' }
    });
  ...
  await t.test('6. Seller Payment Verification: Seller approval confirms order & sets payment_status=verified', async () => {
    const createRes = await makeRequest('/api/orders', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { productId: 'P112', quantity: 3, payment_method: 'upi_qr' }
    });
    const orderId = createRes.body.orders[0].id;
    ...
    await makeRequest(`/api/orders/${orderId}/verify-payment`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { transactionId: 'UTR6677889900' }
    });
  ...
  await t.test('7. IDOR Security: Seller B cannot verify Seller A payment', async () => {
    const createRes = await makeRequest('/api/orders', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${customerToken}`, 'Content-Type': 'application/json' },
      body: { productId: 'P112', quantity: 1, payment_method: 'upi_qr' }
    });
    const orderId = createRes.body.orders[0].id;
